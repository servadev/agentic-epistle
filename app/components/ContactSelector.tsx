      ...
      {selectedContactObjects.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2 max-h-32 overflow-y-auto pr-1">
          {selectedContactObjects.map((contact) => (
            <div
              key={contact.id}
              className="flex items-center gap-1.5 bg-kumo-tint rounded py-1 pl-1 pr-2 border border-kumo-line text-sm max-w-full"
            >
              {contact.avatar_url ? (
                <img
                ...
                <div className="w-5 h-5 rounded-sm bg-slate-200 flex items-center justify-center text-slate-600 shrink-0 text-[10px] font-bold">
                  {contact.name
                    ? contact.name.charAt(0).toUpperCase()
                    : contact.email.charAt(0).toUpperCase()}
                </div>
              )}
              <span className="font-medium text-kumo-strong truncate">
                {contact.name}
              </span>
              <button
                type="button"
            ...
            {filteredContacts.map((contact) => (
              ...
              >
                {contact.avatar_url ? (
                  ...
                  <div className="w-6 h-6 rounded-lg bg-slate-200 flex items-center justify-center text-slate-600 shrink-0 text-xs font-bold shadow-sm">
                    {contact.name
                      ? contact.name.charAt(0).toUpperCase()
                      : contact.email.charAt(0).toUpperCase()}
                  </div>
